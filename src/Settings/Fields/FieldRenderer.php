...
class FieldRenderer
{
	public static function render(string $option_key, FieldDefinition $field, mixed $value): void
	{
		$name = $option_key . '[' . $field->key . ']';
		$id = $option_key . '_' . $field->key;
		...
		switch ($field->field) {
			...
			default:
				throw new \InvalidArgumentException(
					sprintf('Unsupported field type "%s".', esc_html($field->field))
				);
		}
	...
	protected static function media(string $name, mixed $value, FieldDefinition $field): void
	{
		...
		printf(
			...
			esc_html__('Select', Plugin::text_domain()),
			$value ? '' : 'disabled',
			esc_html__('Remove', Plugin::text_domain())
		);
	}
